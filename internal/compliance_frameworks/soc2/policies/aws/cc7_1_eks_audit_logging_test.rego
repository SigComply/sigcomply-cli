package sigcomply.soc2.cc7_1_eks_audit_logging_test

import data.sigcomply.soc2.cc7_1_eks_audit_logging

test_logging_disabled_violation if {
	result := cc7_1_eks_audit_logging.violations with input as {
		"resource_type": "aws:eks:cluster",
		"resource_id": "arn:aws:eks:us-east-1:123:cluster/dev",
		"data": {"name": "dev", "logging_enabled": false},
	}
	count(result) == 1
}

test_logging_enabled_pass if {
	result := cc7_1_eks_audit_logging.violations with input as {
		"resource_type": "aws:eks:cluster",
		"resource_id": "arn:aws:eks:us-east-1:123:cluster/prod",
		"data": {"name": "prod", "logging_enabled": true},
	}
	count(result) == 0
}

test_wrong_resource_type if {
	result := cc7_1_eks_audit_logging.violations with input as {
		"resource_type": "aws:s3:bucket",
		"resource_id": "arn:aws:s3:::bucket",
		"data": {"logging_enabled": false},
	}
	count(result) == 0
}

test_empty_data if {
	result := cc7_1_eks_audit_logging.violations with input as {
		"resource_type": "aws:eks:cluster",
		"resource_id": "arn:aws:eks:us-east-1:123:cluster/test",
		"data": {},
	}
	count(result) == 0
}
