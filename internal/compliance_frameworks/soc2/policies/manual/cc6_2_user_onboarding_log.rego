...
violations contains violation if {
	...
	violation := {
		...
		"reason": sprintf("User onboarding log for period %s is overdue and not uploaded", [input.data.period]),
		"details": {
...
violations contains violation if {
	...
	violation := {
		"resource_id": input.resource_id,
		"resource_type": input.resource_type,
		"reason": "User onboarding log evidence failed integrity verification",
		"details": {
			"evidence_id": input.data.evidence_id,
			"period": input.data.period,
		},
