...
violations contains violation if {
	...
	violation := {
		...
		"reason": sprintf("Emergency change procedure declaration for period %s is overdue and not completed", [input.data.period]),
		"details": {
...
violations contains violation if {
	...
	violation := {
		"resource_id": input.resource_id,
		"resource_type": input.resource_type,
		"reason": "Emergency change procedure declaration was not accepted",
		"details": {
			"evidence_id": input.data.evidence_id,
			"period": input.data.period,
		},
...
violations contains violation if {
	...
	violation := {
		"resource_id": input.resource_id,
		"resource_type": input.resource_type,
		"reason": "Emergency change procedure evidence failed integrity verification",
		"details": {
			"evidence_id": input.data.evidence_id,
			"period": input.data.period,
		},
