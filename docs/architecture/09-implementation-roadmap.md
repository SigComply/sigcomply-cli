...
| Item | Scope | Verification |
|---|---|---|
| **`Slot.Accepts` migration** | Replace `core.Slot.Type string` with `core.Slot.Accepts []string`. Update spec parser to expect `accepts: [...]` in policy YAML (no transitional acceptance of the old `type:` form — pre-M6, no live user data to migrate). Update planner's binding-vs-slot type check to use `source.Emits() ∩ slot.Accepts ≠ ∅`. Update Rego/Go rule input to expose `record.Type` so multi-type slots can switch on it. | All shipped policy specs use `accepts:`; planner unit test verifies the intersection logic; one shipped policy exists with a multi-element `accepts:` list (e.g. cross-cloud object storage). |
| **Self-registering source factories** | Replace the hardcoded `switch id { ... }` in `cmd/sigcomply/check.go` with a generic dispatch through a process-global `sources.Registry`. Each in-tree source package gains an `init()` calling `sources.RegisterFactory(id, factoryFn)`. Orchestrator imports source packages purely for the `init()` side effect (blank imports). | `check.go` contains no per-source case statement; adding a new in-tree source is a single blank-import line plus the package itself. Unit test verifies registration happens at process start without an explicit list. |
| **Evidence-type registry + schema validation** | Introduce `internal/evidence_types/schemas/<id>.v<n>.json` files (JSON Schema documents with extension fields `title`/`version` used by the loader) embedded via `go:embed`. New `EvidenceTypeRegistry` in L2. Collector validates every emitted `EvidenceRecord.Payload` against the registered schema before signing; the first non-conforming record errors the binding; unregistered `Type` is a planning error (exit 3). Project-local types under `.sigcomply/evidence_types/` deferred to M16. The M6 validator implements a minimal Draft-07 subset (`type: object`, `required`, per-property scalar `type`); richer keywords (`format`, `enum`, `pattern`, `items`, composition) are advisory until a future milestone. | Reference schemas shipped for the M6 seed types (`user_record`, `signed_document`, plus at least one cloud resource type for the multi-type slot demo). Validator test demonstrates a malformed payload errors the binding. |
...
| **Plugin set v1** | `aws.s3`, `aws.cloudtrail`, `aws.kms`, `aws.rds`, `aws.ec2`, `aws.cloudwatch`, `aws.guardduty`, `aws.config`, `aws.eks`, `gcp.iam`, `gcp.storage`, `gcp.compute`, `gcp.sql`, `github`, `okta`. Each registers via the factory pattern above. | Each plugin has manifest, tests, at least one shipped policy consuming it. |
...
| **`sigcomply build` (M16)** | Project-local Go-extension build wrapper. Scans `.sigcomply/plugins/*/plugin.go` and `.sigcomply/policies/*/rule.go`; generates a wrapper that blank-imports each project-local package (their `init()` factories register into the same registries the in-tree code uses); invokes `go build` to produce a project-tailored binary. Same mechanism extends to project-local evidence types under `.sigcomply/evidence_types/<id>.v<n>.json` (loaded at orchestrator bootstrap, no Go required). | Fixture project's custom plugin (with a project-local evidence type and a custom policy consuming it) compiles via `sigcomply build` and the resulting binary runs end-to-end. |
