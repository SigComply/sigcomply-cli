...
All collectors are read-only (Describe/List/Get). To check accounts against the organization's people, designate the directory that holds them with `experimental.roster.source` (`okta`, `azure.entra`, `gcp.directory` or `active_directory`); without it the roster policies skip. `aws.identity_center` emits `directory_user`, `roster_entry` **and** `iam_binding` (from permission-set assignments), so on an AWS estate it feeds the least-privilege grant policies too; that traversal needs the `sso:List*`/`DescribePermissionSet` and `identitystore:DescribeGroup`/`ListGroupMemberships` read actions beyond `ListUsers` + `ListInstances` — a roster-only binding needs neither. See [Identity roster](guides/identity-roster.md). For AWS, attach `ReadOnlyAccess` or a scoped read policy to the assumed role. Detailed per-source RBAC is in [configuration.md](configuration.md).
